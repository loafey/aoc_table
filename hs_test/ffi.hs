...
foreign import ccall "lib.h add_func_i32" rust_add_func_i32 ::
                            Ptr Int ->
                            FunPtr (CInt -> CInt) ->
                            FunPtr (CInt -> CInt) ->
                            IO (Ptr Int)
...
addCIntFuncs (TableGen ptr) p1 p2 = do
  ...
  TableGen $ rust_add_func_i32 ptr' p1' p2'

main :: IO ()
main = do
    let table = createTable "Howdy from Haskell!"
    let table' = addCIntFuncs (addCIntFuncs table (\x -> sum [0..100]) (const 421)) (\x -> sum [0..1000]) (const 4121)
    runTable table'
